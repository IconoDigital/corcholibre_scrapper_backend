...
if(isset($_REQUEST["name"]) && $_REQUEST["name"]!='') {
  ...
  $sql="INSERT INTO father_urls (url,source) VALUES ('".$_REQUEST["name"]."','vivanda') ON DUPLICATE KEY UPDATE source='vivanda'";
  mysqli_query($mysqli,$sql) or die(mysqli_error($mysqli));
  $reload=0;
  include_once('simple_html_dom.php');
  $e=0;
  if(file_exists('vivanda_list.html')) {
    unlink("vivanda_list.html");
  }
  //echo "/usr/local/bin/phantomjs fetch_plazavea_list.js ".$_REQUEST["name"]."#".$_SESSION["pagination"]."";
  
  exec("/usr/local/bin/phantomjs fetch_vivanda_list.js ".$_REQUEST["name"]."#".$_SESSION["pagination"]."",$o,$e);
  if($e==0) {
	$links=array();
	$link='vivanda_list.html';
	$html = file_get_html($link); // Create DOM from URL or file
	$counter=0;
	foreach($html->find(".products ul") as $ul) {
      foreach($ul->find("li") as $li) {
	    foreach($li->find(".img-product") as $figure) {
	...
	/*foreach($html->find(".box-img-product") as $box){
	  foreach($box->find(".img-product") as $figure) {
		foreach($figure->find("a") as $anchor){
		  $links[]=$anchor->href;
		}
	  }	  
	...
	if($counter==0) {
	  ...
	  $sql.=" ON DUPLICATE KEY UPDATE `source`='vivanda'";
...
<head>
    ...
    <title>Add Vivanda Product List</title>
                  ...
                  <div class="col-lg-12">
                      <section class="panel">
						  ...
						  <form action="" accept-charset="utf-8" method="post">
							...
							<br>
							<br>
							 &nbsp &nbsp Vivanda Father Link: <input type="text" name="name" > &nbsp <input type="submit" name="submit" value="Submit"> 
							 </form>
                          </div>
                      </section>
                  </div>
